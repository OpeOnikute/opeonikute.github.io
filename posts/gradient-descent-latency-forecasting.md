...
A few weeks ago I published a video that walks through **gradient descent** in the context of something SREs actually care about: **whether we can forecast load and latency** well enough to act before customers feel pain. If you prefer video, the full walkthrough is here: [Gradient descent for traffic forecasting](https://www.youtube.com/watch?v=vNDvFW2pOxQ).
...
All of the experiments live in a notebook in my [autoscaling-playground](https://github.com/OpeOnikute/autoscaling-playground) repo: [`notebooks/traffic_forecasting.ipynb`](https://github.com/OpeOnikute/autoscaling-playground/blob/main/notebooks/traffic_forecasting.ipynb).

## What we are trying to predict

The notebook uses rows from a **microservices bottleneck-detection** style dataset: per time window you get aggregate **CPU**, **network RX/TX**, and **latency**. The broad aim is capacity planning — if CPU and traffic are increasing on a smooth trend, can we translate that combination of load into an expected latency?
...
## Combining two models for prediction
...
## Why Model 2 is trained only on the baseline
...
## What we've learned from this exploration
...
- Video: [youtube.com/watch?v=vNDvFW2pOxQ](https://www.youtube.com/watch?v=vNDvFW2pOxQ)
- Notebook: [github.com/OpeOnikute/autoscaling-playground/blob/main/notebooks/traffic_forecasting.ipynb](https://github.com/OpeOnikute/autoscaling-playground/blob/main/notebooks/traffic_forecasting.ipynb)
